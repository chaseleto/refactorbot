...
class Admin(commands.Cog):
    ...
    async def info(self, ctx, member: discord.Member):
        embed = discord.Embed(title=f"{member.name}#{member.discriminator}", description=f"{member.mention} ({member.id})", color=0x00ff00)
        embed.add_field(name="Joined At", value=member.joined_at.strftime("%a, %#d %B %Y, %I:%M %p UTC"), inline=False)
        embed.add_field(name="Roles", value=", ".join([role.mention for role in member.roles if role != ctx.guild.default_role]), inline=False)
    ...
    async def server(self, ctx):
        embed = discord.Embed(title=f"{ctx.guild.name} ({ctx.guild.id})", description=ctx.guild.description, color=0x00ff00)
    ...
    async def server(self, interaction: discord.Interaction):
        embed = discord.Embed(title=f"{interaction.guild.name} ({interaction.guild.id})", description=interaction.guild.description, color=0x00ff00)
        embed.add_field(name="Owner", value=interaction.guild.owner.mention, inline=False)
        embed.add_field(name="Created At", value=interaction.guild.created_at.strftime("%a, %#d %B %Y, %I:%M %p UTC"), inline=False)
        ...
        embed.add_field(name="Channels", value=len(interaction.guild.channels), inline=False)
